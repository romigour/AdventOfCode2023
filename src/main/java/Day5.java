...
public class Day5 {
    ...
    private static long reponse2(List<String> inputs) {
        ...
        for (int r = 0; r < rangeSeeds.size(); r = r+2) {
            ...
            for (long j = rangeSeeds.get(r); j < (rangeSeeds.get(r) + rangeSeeds.get(r+1)); j++) {
                seeds.add(j);
            }
            ...
            survivors.add(seeds.stream().mapToLong(v -> v).min().orElse(Long.MAX_VALUE));
        }

        return survivors.stream().mapToLong(v -> v).min().orElseThrow();
    }
}
